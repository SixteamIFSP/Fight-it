...
const RenderEvaluation = ({ item, data, selectEvaluation, setSelectEvaluation }) => {
    ...
    return (
        <EvaluationSelect select={item?.id === selectEvaluation} onPress={() => handleTouch()}>
            ...
            <Text>{t('Nome: ' + item?.nome)}</Text>
            <Text>{t('data: ' + item?.criação.slice(0, 10))}</Text>
        </EvaluationSelect>
    )
}
...
function CreatePerformace({ dataParams, setCreatePerformace }) {
    ...
    function handleSubmit() {
        if (nomeDesempenho !== '' & dataCriacao !== null) {
            ...
            {t(toastMessage(false, 'Preencha corretamente os campos'))}
        }
    }
    function handleBack() {
        setCreatePerformace(false);
    ...
    return (
        <View>
            ...
            <Input
                ...
                placeholder={t("Nome do desempenho")}
            />
            <Input
                value={dataCriacao}
                editable={false}
...
function FormCreateParams({ selectEvaluation, setSelectEvaluation }) {
    ...
    return (
        <View>
            <TextHeader>{t("Criação de parâmetro:")}</TextHeader>
            <AlingDropDown>
            ...
            <AlingButtons>
                ...
                {!createParams && <Button handle={() => handleSubmit()} text={t("Enviar Avaliação")} />}


                <Button handle={() => handleCreateParam()} text={t("Criar Novo parâmetro")} />
